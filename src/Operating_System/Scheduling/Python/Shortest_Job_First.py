...
def waiting_time_util(processes):
	...
	completed_jobs = 0
	completed_jobs_array = [False for _ in range(n)]

	shortest = 0
	shortest_remaining = processes[shortest].burst_time
	flag = True
	...
	while completed_jobs != n:
		...
		if shortest_remaining <= 0:
			...
			if flag == True: # flag here to check for repetative block assessment for 1 job
				completed_jobs += 1
				completed_jobs_array[shortest] = True
				
				processes[shortest].completion_time = clock_ticks
			...
			check = False
			for i in range(n):
				if (completed_jobs_array[i] == False):
					if (processes[i].arrival_time <= clock_ticks
					and processes[i].burst_time < minimum):
			...
			continue # no task done so dont increment clock
